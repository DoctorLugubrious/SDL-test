#ifndef CONSTANTS_DOT_H_
#define CONSTANTS_DOT_H_
	
const int WINDOW_WIDTH = 800;
const int WINDOW_HEIGHT = 600;

const int X_LIMIT = WINDOW_WIDTH - 20;
const int Y_LIMIT = WINDOW_HEIGHT;
const int X_MIN = -50;
const int X_ACCELERATION = 2;
const int Y_ACCELERATION = 35;

const int X_VELOCITY_MAX = 15;
const int Y_VELOCITY_MAX = 100;

const int GRAVITY = 2;

static const int WALK_SPRITE_WIDTH = 57;
static const int STAND_SPRITE_WIDTH = 37;
static const int SPRITE_HEIGHT = 57;

static const int BS = 0;

enum CharacterState {STAND_LEFT, STAND_RIGHT, WALK_LEFT, WALK_RIGHT,
        JUMP_LEFT, JUMP_RIGHT, DUCK_LEFT, DUCK_RIGHT, RANGED_ATTACK_LEFT,
        RANGED_ATTACK_RIGHT, AURA_SPHERE_LEFT, AURA_SPHERE_RIGHT, HURT_LEFT,
	HURT_RIGHT, DUCK_ATTACK_LEFT, DUCK_ATTACK_RIGHT, AIR_ATTACK_LEFT, 
	AIR_ATTACK_RIGHT};

#endif
